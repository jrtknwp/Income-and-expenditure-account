...
let db; let screen = "home"; let editingId = null; let draft = newDraft(); let receiptBlob = null; let receiptPreviewUrl = ""; let ocrState = { status:"idle", message:"", rawText:"", debug:null }; let historyFilters = { query:"", type:"all", category:"all", date:"" }; let summaryDate = new Date(); let summaryMode = "daily"; let summaryDay = localDate(); let categoryReturnScreen = "form"; let categoryEditor = { id:"", icon:"✨", name:"" };
    ...
    return Tesseract.createWorker("tha", 1, {
      ...
      logger: (event) => {
        if (event.status === "recognizing text" && typeof event.progress === "number") {
          if (workerRecognizingRunId === activeOcrRunId && ocrState.status === "processing") {
            ocrState.message = `กำลังอ่านข้อความสลิปในอุปกรณ์… ${Math.round(event.progress * 100)}%`;
            const now = performance.now();
            if (screen === "import" && now - lastOcrProgressRender >= 250) { lastOcrProgressRender = now; renderForm(true); }
          }
        }
      }
    });
...
function assertImageDimensions(image) { const { width, height } = imageDimensions(image); if (!width || !height) throw new Error("รูปสลิปมีขนาดไม่ถูกต้อง"); return { width, height }; }
async function receiptBitmap(file) {
  if ("createImageBitmap" in window) {
    const bitmap = await createImageBitmap(file, { imageOrientation:"from-image" }); assertImageDimensions(bitmap); return bitmap;
  }
  const url = URL.createObjectURL(file);
  try {
    const image = new Image(); image.src = url;
    if (typeof image.decode === "function") await image.decode(); else await new Promise((resolve, reject) => { image.onload=resolve; image.onerror=()=>reject(new Error("เปิดรูปสลิปไม่สำเร็จ")); });
    assertImageDimensions(image); return image;
  } finally { URL.revokeObjectURL(url); }
}
function resizedReceiptCanvas(bitmap, maxEdge = 2048) {
  const { width, height } = assertImageDimensions(bitmap); const scale = Math.min(1, maxEdge / Math.max(width, height));
  const canvas = document.createElement("canvas"); canvas.width = Math.max(1, Math.round(width * scale)); canvas.height = Math.max(1, Math.round(height * scale));
  const context = canvas.getContext("2d", { willReadFrequently:true }); if (!context) throw new Error("สร้าง canvas สำหรับ OCR ไม่สำเร็จ");
  context.drawImage(bitmap, 0, 0, width, height, 0, 0, canvas.width, canvas.height); return canvas;
}
function preparedCrop(source, box, contrast = 1.45) {
  const { width:sourceWidth, height:sourceHeight } = assertImageDimensions(source);
  const sx = Math.max(0, Math.min(sourceWidth - 1, Math.round(sourceWidth * box.x))); const sy = Math.max(0, Math.min(sourceHeight - 1, Math.round(sourceHeight * box.y)));
  const sw = Math.max(1, Math.min(sourceWidth - sx, Math.round(sourceWidth * box.w))); const sh = Math.max(1, Math.min(sourceHeight - sy, Math.round(sourceHeight * box.h)));
  const scale = Math.min(2.5, 2200 / Math.max(sw, 1)); const canvas = document.createElement("canvas"); canvas.width = Math.max(1, Math.round(sw * scale)); canvas.height = Math.max(1, Math.round(sh * scale));
  const context = canvas.getContext("2d", { willReadFrequently:true }); if (!context) throw new Error("สร้าง crop canvas ไม่สำเร็จ"); context.drawImage(source, sx, sy, sw, sh, 0, 0, canvas.width, canvas.height);
  const pixels = context.getImageData(0, 0, canvas.width, canvas.height);
  for (let index=0; index<pixels.data.length; index+=4) { const gray = (pixels.data[index] * .299) + (pixels.data[index+1] * .587) + (pixels.data[index+2] * .114); const boosted = Math.max(0, Math.min(255, (gray - 128) * contrast + 128)); pixels.data[index] = pixels.data[index+1] = pixels.data[index+2] = boosted; }
  context.putImageData(pixels, 0, 0); return canvas;
}
function releaseCanvas(canvas) { canvas.width = 0; canvas.height = 0; }
async function recognizePass(worker, image, name, attempt) {
  try { const result = await worker.recognize(image); const text = result.data?.text || ""; attempt.passes.push({ name, success:true, textLength:text.length }); return text; }
  catch (error) { const message=error?.message || String(error); attempt.passes.push({ name, success:false, error:message }); console.error(`OCR ${name} failed`, error); return ""; }
}
async function ocrPasses(source, worker, attemptNumber) {
  const retry = attemptNumber > 1; const attempt = { number:attemptNumber, passes:[] }; const crops = retry ? [
    { name:"AMOUNT CROP OCR (retry upper-middle)", box:{ x:.04, y:.11, w:.92, h:.46 }, type:"amount" }, { name:"AMOUNT CROP OCR (retry lower-middle)", box:{ x:.03, y:.40, w:.94, h:.52 }, type:"amount" }, { name:"DATE CROP OCR (retry top)", box:{ x:.02, y:.01, w:.96, h:.42 }, type:"date" }
  ] : [
    { name:"AMOUNT CROP OCR (upper-middle)", box:{ x:.08, y:.15, w:.84, h:.38 }, type:"amount" }, { name:"AMOUNT CROP OCR (lower-middle)", box:{ x:.06, y:.45, w:.88, h:.45 }, type:"amount" }, { name:"DATE CROP OCR", box:{ x:.04, y:.02, w:.92, h:.34 }, type:"date" }
  ];
  const fullCanvas = retry ? preparedCrop(source, { x:0, y:0, w:1, h:1 }, 1.7) : source; const full = await recognizePass(worker, fullCanvas, `FULL OCR${retry ? " (retry)" : ""}`, attempt); if (fullCanvas !== source) releaseCanvas(fullCanvas);
  const results = { full, amount:[], date:"", attempt };
  for (const crop of crops) { const canvas = preparedCrop(source, crop.box, retry ? 1.8 : 1.45); try { const text = await recognizePass(worker, canvas, crop.name, attempt); if (crop.type === "amount") results.amount.push({ name:crop.name, text }); else results.date = text; } finally { releaseCanvas(canvas); } }
  return results;
}
function ocrValues(passes) { const parsed = parseOcrText(passes.full); const chosenAmount = chooseAmount(passes.full, passes.amount, passes.date); return { parsed, chosenAmount, finalDate:extractSlipDate(passes.date) || extractSlipDate(passes.full) }; }
async function performOcr(file, runId) {
  const debug = { workerReady:false, image:null, attempts:[], bank:"ไม่ระบุ", full:"", amountCrops:[], dateCrop:"", finalAmount:"ไม่พบ", finalDate:"ไม่พบ", amountCandidates:[], error:"" };
  let bitmap; let source;
  try {
    bitmap = await receiptBitmap(file); const original = imageDimensions(bitmap); source = resizedReceiptCanvas(bitmap); debug.image = { original, normalized:imageDimensions(source), fileBytes:file.size };
    const worker = await getOcrWorker(); debug.workerReady = true; workerRecognizingRunId = runId;
    const first = await ocrPasses(source, worker, 1); debug.attempts.push(first.attempt); let values = ocrValues(first); let allAmount = [...first.amount]; let finalDate = values.finalDate; let fullText = first.full; let retry = null;
    if (!values.chosenAmount.amount || !finalDate) { retry = await ocrPasses(source, worker, 2); debug.attempts.push(retry.attempt); const retryValues = ocrValues(retry); allAmount = [...allAmount, ...retry.amount]; const combined = { full: `${first.full}\n${retry.full}`, amount:allAmount, date:`${first.date}\n${retry.date}` }; values = { parsed:parseOcrText(combined.full), chosenAmount:chooseAmount(combined.full, combined.amount, combined.date), finalDate:extractSlipDate(retry.date) || finalDate || extractSlipDate(combined.full) }; fullText = combined.full; }
    if (runId !== activeOcrRunId) return;
    Object.assign(draft, Object.fromEntries(Object.entries(values.parsed.fields).filter(([key, value]) => value && !["amount", "date"].includes(key)))); draft.amount = values.chosenAmount.amount; draft.date = values.finalDate;
    Object.assign(debug, { bank:detectBank(`${fullText} ${first.date} ${retry?.date || ""}`), full:fullText, amountCrops:allAmount, dateCrop:`${first.date}${retry ? `\n${retry.date}` : ""}`, finalAmount:draft.amount || "ไม่พบ", finalDate:draft.date || "ไม่พบ", amountCandidates:values.chosenAmount.candidates });
    ocrState = { status:"done", message:`อ่านสลิปเสร็จแล้ว${draft.amount ? ` · พบยอด ${money(draft.amount)}` : " · ไม่พบยอดเงิน"}${draft.date ? "" : " · ไม่พบวันที่จากสลิป"}`, rawText:fullText, debug };
  } catch (error) {
    if (runId !== activeOcrRunId) return; debug.error=error?.message || String(error); console.error(error); ocrState = { status:"error", message:`OCR ไม่สำเร็จ (${debug.error})`, debug };
  } finally { if (source) releaseCanvas(source); if (bitmap && typeof bitmap.close === "function") bitmap.close(); if (workerRecognizingRunId === runId) workerRecognizingRunId = 0; }
  if (runId === activeOcrRunId) renderForm(true);
}
async function runOcr(file) {
  if (ocrState.status === "processing") return; const runId = ++activeOcrRunId; lastOcrProgressRender = 0; ocrState = { status:"processing", message:"กำลังเตรียม OCR ภาษาไทยในอุปกรณ์…", debug:{ workerReady:false, attempts:[] } }; screen = "import"; renderForm(true);
  const job = ocrQueue.catch(() => {}).then(() => performOcr(file, runId)); ocrQueue = job.catch(() => {}); return job;
}
function clearReceiptPreview() { if (receiptPreviewUrl) URL.revokeObjectURL(receiptPreviewUrl); receiptPreviewUrl = ""; }
function receiptPreviewSource() { if (!receiptBlob) { clearReceiptPreview(); return ""; } if (!receiptPreviewUrl) receiptPreviewUrl = URL.createObjectURL(receiptBlob); return receiptPreviewUrl; }
...
function ocrStatusHtml() { const d=ocrState.debug; const cropText=d?.amountCrops?.map((pass)=>`${pass.name}\n${pass.text || "ไม่มีข้อความที่อ่านได้"}`).join("\n\n") || "ไม่มีข้อความที่อ่านได้"; const candidates=d?.amountCandidates?.map((candidate)=>`${candidate.amount} (${candidate.source})`).join(", ") || "ไม่มี"; const passStatus=d?.attempts?.map((attempt)=>`รอบ ${attempt.number}: ${attempt.passes.map((pass)=>`${pass.name} ${pass.success ? "✓" : `✗ ${pass.error || ""}`}`).join(" · ")}`).join("\n") || "กำลังเตรียม"; const debug=d ? `<details class="ocr-debug"><summary>Debug OCR (local-only)</summary><dl><dt>IMAGE</dt><dd>${d.image ? `${d.image.original.width}×${d.image.original.height} → ${d.image.normalized.width}×${d.image.normalized.height}px · ${d.image.fileBytes} bytes` : "กำลังรอรูป"}</dd><dt>WORKER READY</dt><dd>${d.workerReady ? "yes" : "no"}</dd><dt>OCR PASS</dt><dd><pre>${escapeHtml(passStatus)}</pre></dd><dt>BANK</dt><dd>${escapeHtml(d.bank || "ไม่ระบุ")}</dd><dt>FINAL AMOUNT</dt><dd>${escapeHtml(d.finalAmount || "ไม่พบ")} · candidates: ${escapeHtml(candidates)}</dd><dt>FINAL DATE</dt><dd>${escapeHtml(d.finalDate || "ไม่พบ")}</dd><dt>ERROR</dt><dd>${escapeHtml(d.error || "ไม่มี")}</dd></dl><h3>FULL OCR</h3><pre>${escapeHtml((d.full || "").slice(0,12000) || "ไม่มีข้อความที่อ่านได้")}</pre><h3>AMOUNT CROP OCR</h3><pre>${escapeHtml(cropText.slice(0,12000))}</pre><h3>DATE CROP OCR</h3><pre>${escapeHtml((d.dateCrop || "").slice(0,12000) || "ไม่มีข้อความที่อ่านได้")}</pre></details>` : ""; if (ocrState.status === "processing") return `<div class="ocr-status processing">${escapeHtml(ocrState.message)}${debug}</div>`; if (ocrState.status === "done") return `<div class="ocr-status success">${escapeHtml(ocrState.message)}${debug}</div>`; if (ocrState.status === "error") return `<div class="ocr-status warning">${escapeHtml(ocrState.message)}${debug} กรุณากรอกหรือแก้ไขข้อมูลด้านล่าง</div>`; return ''; }
async function renderForm(importing=false) { const confirmLabel=importing?'ยืนยันและบันทึก':'บันทึกรายการ'; const ocrBusy=ocrState.status==='processing'; const receiptAction=receiptBlob?`<img class="receipt-preview" src="${receiptPreviewSource()}" alt="รูปสลิปที่เลือก" /><button class="secondary" type="button" data-change-receipt ${ocrBusy?'disabled':''}>เปลี่ยนรูปสลิป</button>`:`<button class="secondary" type="button" data-import ${ocrBusy?'disabled':''}>📷 เลือกรูปสลิป</button>`; app.innerHTML=`<header class="page-head"><button class="back" data-nav="home">←</button><div><p class="eyebrow">${importing?'ตรวจสอบก่อนบันทึก':'บันทึกให้เร็ว'}</p><h1>${importing?'ตรวจสอบรายการ':'เพิ่มรายการ'}</h1></div></header><form id="transaction-form" class="form-card"><div class="type-toggle"><button type="button" data-type="expense" class="${draft.type==='expense'?'selected':''}">รายจ่าย</button><button type="button" data-type="income" class="${draft.type==='income'?'selected':''}">รายรับ</button></div>${importing?`${ocrStatusHtml()}<p class="hint">OCR เป็นเพียงข้อมูลเสนอแนะ โปรดตรวจสอบ แก้ไข และเลือกหมวดหมู่ด้วยตัวเองก่อนบันทึก</p>`:''}<label class="field">จำนวนเงิน<input class="amount-input" name="amount" type="number" inputmode="decimal" min="0.01" step="0.01" placeholder="0.00" value="${draft.amount}" required autofocus /></label><label class="field">เลือกหมวดหมู่${categoryButtons()}</label><label class="field">ชื่อร้าน / ผู้รับเงิน<input name="merchant" placeholder="เช่น 7-Eleven" value="${escapeHtml(draft.merchant)}" /></label><div class="filter-grid"><label class="field">วันที่<input name="date" type="date" value="${draft.date}" required /></label><label class="field">เวลา<input name="time" type="time" value="${draft.time}" /></label></div>${importing?`<label class="field">เลขอ้างอิง (ถ้ามี)<input name="reference" placeholder="เลขอ้างอิงจากสลิป" value="${escapeHtml(draft.reference || '')}" /></label>`:''}<label class="field">หมายเหตุ (ไม่บังคับ)<textarea name="note" rows="2" placeholder="เพิ่มรายละเอียดได้">${escapeHtml(draft.note)}</textarea></label>${receiptAction}<button class="primary save" type="submit">${confirmLabel}</button>${editingId?'<button class="secondary save" type="button" data-cancel-edit>ยกเลิก</button>':''}</form>`; }
...
receiptInput.addEventListener("change", async () => { const file=receiptInput.files[0]; receiptInput.value=""; if(!file || ocrState.status === "processing") return; clearReceiptPreview(); receiptBlob=file; draft={...newDraft(), category:"", date:"", source:"receipt"}; toast("แนบรูปแล้ว กำลังอ่านข้อความสลิป…"); await runOcr(file); });
